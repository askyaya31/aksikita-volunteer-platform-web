...
<div class="chat-layout">
    ...
    <div class="sidebar">
        ...
        <div class="rooms">
            @php
                $allRooms = \App\Models\ChatRoom::where('organizer_id', session('user_id'))
                    ->with(['volunteer.volunteerProfile', 'latestMessage', 'event'])
                    ->latest('updated_at')
                    ->get();
            @endphp
            ...
            @foreach($allRooms as $r)
                @php
                    $rVolProfile = $r->volunteer->volunteerProfile;
                    $rVolName    = $r->volunteer->name;
                    $rVolAvatar  = $rVolProfile->avatar ?? null;
                    $rVolInitial = strtoupper(substr($rVolName, 0, 1));
                ...
                <a href="{{ route('organizer.chat.show', $r) }}"
                   class="room-item {{ $r->id === $room->id ? 'active' : '' }}">
                    <div class="avatar">
                        @if($rVolAvatar)
                            <img src="{{ Storage::url($rVolAvatar) }}" alt="{{ $rVolName }}">
                        @else
                            {{ $rVolInitial }}
                        @endif
                        @if($rHasUnread)
                    ...
                    <div class="room-info">
                        <div class="room-top">
                            <div class="room-name {{ $rHasUnread ? 'bold' : '' }}">{{ $rVolName }}</div>
                            @if($rTimestamp)
    ...
    <div class="chat-panel">
        <div class="chat-header">
            ...
            <div class="chat-header-info">
                <div class="chat-title">{{ $volName }}</div>
                <div class="chat-sub">
                    <span class="status-dot"></span>
                    {{ $room->event->title }}
                </div>
            </div>
            <a href="{{ route('organizer.chat.index') }}" class="header-btn" title="Kembali">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M19 12H5M12 19l-7-7 7-7"/>
                </svg>
            </a>
            <div class="header-btn" title="Info relawan">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    function appendBubble(data) {
        ...
        if (data.is_me) {
            meta.innerHTML = `
                ...
                <span>${escHtml(data.created_at)}</span>`;
        } else {
            meta.innerHTML = `<span>${escHtml(data.created_at)}</span>`;
        }
