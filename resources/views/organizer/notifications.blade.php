...
.notif-empty p  { font-size: 13px; color: #94A3B8; max-width: 300px; margin: 0 auto; }
.notif-pagination {
    display: flex;
    align-items: center;
    justify-content: center;
...
$iconMap = [
    ...
    'event_pending_review'   => ['bg' => '#EFF6FF', 'color' => '#3B82F6', 'icon' => 'clock'],
    'registration_confirmed' => ['bg' => '#DBEAFE', 'color' => '#1E3A8A', 'icon' => 'check'],
    'registration_rejected'  => ['bg' => '#F1F5F9', 'color' => '#64748B', 'icon' => 'x'],
    'attendance_confirmed'   => ['bg' => '#DBEAFE', 'color' => '#1E3A8A', 'icon' => 'check'],
    'event_completed'        => ['bg' => '#EFF6FF', 'color' => '#3B82F6', 'icon' => 'star'],
    ...
    <div class="ak-container">
        <div class="notif-topbar">
            <div>
                ...
                <p class="notif-subhead">Aktivitas terbaru pada event dan kandidat kamu</p>
            </div>
            ...
            @foreach($grouped as $groupLabel => $items)
                ...
                <div class="notif-list">
                    @foreach($items as $notif)
                        @php $meta = $iconMap[$notif->type] ?? ['bg' => '#F1F5F9', 'color' => '#64748B', 'icon' => 'bell']; @endphp
                        <div class="notif-item {{ $notif->is_read ? 'is-read' : 'is-unread' }}">
                            <div class="notif-icon" style="background:{{ $meta['bg'] }};">
                                <svg width="15" height="15" fill="none" viewBox="0 0 24 24"
                                     stroke="{{ $meta['color'] }}" stroke-width="2.5" stroke-linecap="round">
                                    @if($meta['icon'] === 'check')
                                        <polyline points="20 6 9 17 4 12"/>
                                    @elseif($meta['icon'] === 'x')
